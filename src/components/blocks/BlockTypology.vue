<script setup>
  ...
  useTitle(`planner.express // NEW PROJECT`)
  ...
  onMounted(() => {
    stProjects.proj.authorId = stAuth.profile.uid
    stProjects.proj.isNew = true
    stProjects.proj.pendingUpdate = true
    // stUser.getFSDocs(['projects'], 'projects')
  })

  const objectToString = computed(() => {
    return Object.entries(stProjects.proj)
